...
final class Promokodiki_Admitad_Review_Page {
	...
	public static function table_context( array $input ): array {
		...
		if ( isset( $groups[ $reason ] ) ) { $filters['reason'] = ''; }
		...
		if ( isset( $groups[ $reason ] ) ) { $rows['items'] = array_values( array_filter( $rows['items'], static fn( array $row ): bool => in_array( $row['reason_code'], $groups[ $reason ], true ) ) ); $rows['total'] = count( $rows['items'] ); }
		...
		unset( $row ); $terms = get_terms( array( 'taxonomy' => 'promocode_category', 'hide_empty' => false, 'orderby' => 'name' ) ); $totals = array(); foreach ( $groups as $key => $codes ) { $sum = 0; foreach ( $codes as $code ) { $sum += $repository->list_rows( '', 1, 100, array( 'status' => 'open', 'reason' => $code ) )['total']; } $totals[ $key ] = $sum; }
