...
const tabs = new graph_tabs__WEBPACK_IMPORTED_MODULE_1__["default"]('discounts');

// Подключение свайпера


let swiperNews = new swiper__WEBPACK_IMPORTED_MODULE_2__["default"](".news__slider", {
  modules: [swiper_modules__WEBPACK_IMPORTED_MODULE_3__.Navigation, swiper_modules__WEBPACK_IMPORTED_MODULE_3__.Scrollbar, swiper_modules__WEBPACK_IMPORTED_MODULE_3__.Pagination],
...
(function ($) {
  $(document).ready(function () {
    $(document).ready(function () {
      ...
      $('a[href^="#"]').on('click', function (event) {
        ...
        if (target.length) {
          // Анимируем скролл к элементу
          $('html, body').stop().animate({
            scrollTop: target.offset().top
          }, 1000); // Длительность анимации в миллисекундах (1000 = 1 секунда)
        }
      });
    });
    ...
    $('.seo__button').click(function () {
      const $button = $(this);
      const $content = $button.parent('.seo__column').find('.seo__content');
      const isExpanded = $button.text() === 'Развернуть';
      $button.text(isExpanded ? 'Скрыть' : 'Развернуть');
      $content.toggleClass('seo__content_hide', !isExpanded);
    });
    $(document).ready(function () {
      $('.promocodes__copy').on('click', function () {
    ...
    $(document).ready(function () {
      // Изначально скрываем все параграфы кроме первого
      $('.promocodes__shop-text p:not(:first)').hide();
      $('.promocodes__shop-view').on('click', function () {
        // Переключаем класс у кнопки
        $(this).toggleClass('promocodes__shop-view_on');
        $('.promocodes__shop-text').toggleClass('promocodes__shop-text_on');
        // Переключаем видимость всех параграфов кроме первого
        $('.promocodes__shop-text p:not(:first)').toggle();

        // Меняем текст кнопки
        if ($(this).hasClass('promocodes__shop-view_on')) {
          $(this).text('Скрыть');
        } else {
          $(this).text('Подробнее');
        }
  ...
  $(document).ready(function () {
    ...
    function checkScreenSize() {
      if ($(window).width() <= 768) {
        // Если экран мобильный и текст не помещается
        if ($('.promo__text').get(0).scrollWidth > $('.promo__text').innerWidth()) {
          $('.promo__text span').addClass('marquee');
        } else {
          $('.promo__text span').removeClass('marquee');
        }
      } else {
        // На десктопах убираем эффект бегущей строки
        $('.promo__text').removeClass('marquee');
      }
    }
  ...
  $(document).ready(function () {
    ...
    $(window).scroll(function () {
      if ($(this).scrollTop() > 300) {
        ...
        $('.footer__button_up').fadeIn();
      } else {
        $('.footer__button_up').fadeOut();
      }
    });

    // Плавная прокрутка при клике
    $('.footer__button_up').click(function () {
      $('html, body').animate({
        scrollTop: 0
...
document.addEventListener('DOMContentLoaded', function () {
  // Устанавливаем продолжительность таймера - 3 дня в миллисекундах
  const duration = 3 * 24 * 60 * 60 * 1000; // 3 дня в миллисекундах
  const timerElements = {
    days: document.getElementById('topDays'),
    hours: document.getElementById('topHours'),
    minutes: document.getElementById('topMinutes'),
    seconds: document.getElementById('topSeconds')
  };
  if (Object.values(timerElements).some(function (element) { return !element; })) {
    return;
  }

  function updateTimer() {
    // Проверяем, есть ли в localStorage сохраненное время окончания
    let endTime = localStorage.getItem('couponTimerEnd');

    // Если нет сохраненного времени или оно уже прошло, устанавливаем новое
    if (!endTime || Date.now() > endTime) {
      endTime = Date.now() + duration;
      localStorage.setItem('couponTimerEnd', endTime);
    }

    // Вычисляем оставшееся время
    const remainingTime = endTime - Date.now();

    // Если время истекло, запускаем таймер заново
    if (remainingTime <= 0) {
      localStorage.removeItem('couponTimerEnd');
      updateTimer();
      return;
    }
    ...
    timerElements.minutes.textContent = Math.floor(minutes);
