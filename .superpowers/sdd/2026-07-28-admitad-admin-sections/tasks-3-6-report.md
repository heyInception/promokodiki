...
- Disposable DB portability: schema preservation now checks legacy-table existence before counting and proves absent legacy tables remain absent. Coordinator snapshots skip MySQL `information_schema.AUTO_INCREMENT` under SQLite, always return structured state, and restore only captured scoped tables/options/events. Retry/facade cleanup releases only the owner-matched test job lock before removing its owner token. Focused `test-schema.php` passed 2/2 and `test-sync-coordinator.php` passed 5/5 without database errors.
